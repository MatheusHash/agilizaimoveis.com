...

{
    ...
    {
        // Remove todas as imagens do banco
        Galeria::where('imovel_id', $idImovel)->delete();

        // Busca o imóvel
        $imovel = Imovel::findOrFail($idImovel);

        // Se houver relacionamento com tags
        $imovel->tags()->detach();

        // Deleta o imóvel do banco
        $imovel->delete();

        // Caminho físico da pasta de imagens
        if (app()->environment('dev')) {
            $directory = public_path("uploads/{$idImovel}");
        } else {
            $directory = base_path("public_html/uploads/{$idImovel}");
        }

        // Se existir, apaga toda a pasta e tudo dentro dela
        if (File::exists($directory)) {
            File::deleteDirectory($directory);
        }

        return response()->json([
            'success' => 'Imóvel apagado!'
        ], 200);
    }
}
